...
def dateConverter(aDate):
    hyphens = False
    hyphensList = []
    slashList = []
    for item in aDate:
        if item == '-':
            hyphensList.append(1)
        elif item == '/':
            slashList.append(1)

    if len(hyphensList) > 1:
        hyphens = True

    if hyphens:
        answer = hyphensDate(aDate)
    else:
        answer = slashDate(aDate)
...
for stateName in states:
    stateDataFrame[stateName] = pd.DataFrame(casesList[stateName],
                                             columns=['Date', 'Total Cases', 'per Capita', 'Daily Change'])
